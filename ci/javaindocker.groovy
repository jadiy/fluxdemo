node {
  workdir="/bitnami/jenkins/home/workspace/workdir"
...
def InitBuild(){
    dir ("${workdir}"){
        sh "docker build -t ruoyi-init  ."
    }
}

def DockerBuild(){
    dir ("${workdir}/ci"){
        sh "docker build -t ${DOCKER_IMAGE_NAME}:${ENV}.${BUILD_NUMBER} --build-arg ENV=${ENV} --build-arg buildname=${DOCKER_IMAGE_NAME} ."
    }
}
...
def Deploy(){
    stage ('Deploy Env') {
      ...
      sh "kubectl apply -f ${workdir}/cd/${DOCKER_IMAGE_NAME}-k8s-deployment.${ENV}.tmp --kubeconfig=${workdir}/cd/config "
    }
}
